...
fun PlantDetailContent(plant: Plant) {
    Surface {
        Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.margin_normal))) {
            ...
            PlantWater(wateringInterval = plant.wateringInterval)
        }
    }
}
...
private fun PlantDetailContentPreview() {
    val plant = Plant("id", "Apple", "des", 3, 30, "")
    MaterialTheme {
...
@Composable
private fun PlantWater(wateringInterval: Int) {
    Column(Modifier.fillMaxWidth()) {
        var centerWithPaddingModifier =
            Modifier
